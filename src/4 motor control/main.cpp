...
pros::Controller master (CONTROLLER_MASTER);
	...
	while (true)
	{
		int L1  = pros::E_MOTOR_GEARSET_06;

		int distance = ultrasonic.get_value();
			if (distance > 200 || distance <= 0 )
			{
				left_wheels_f  =  master.get_analog(ANALOG_LEFT_Y) / L1;
				left_wheels_b = master.get_analog(ANALOG_LEFT_Y) / L1;
				right_wheels_f  = master.get_analog(ANALOG_RIGHT_Y) / L1;
				right_wheels_b = master.get_analog(ANALOG_RIGHT_Y) / L1;
				pros::delay(50);
			}
			else
			{
